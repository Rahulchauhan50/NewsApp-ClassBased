...
export class News extends Component {
    ...
    async componentDidMount(){
        ...
        await fetch(`https://newsapi.org/v2/top-headlines?country=${this.state.Country}&category=${this.state.Category}&apiKey=1e0a9b492d664dd1b3751ee483316063&pageSize=6&page=${this.state.articles.page}`)
        .then((data)=>{
            return data.json()
        })
        .then((respone)=>{
            this.setState({
                ...
                laoding: false,
                totalPages : respone.totalResults%6==0?respone.totalResults/6:((respone.totalResults/6)+1)-((respone.totalResults%6)/6)
            })
    ...
    HandleOnPrev = async () => {
        this.setState({laoding: true})
        await fetch(`https://newsapi.org/v2/top-headlines?country=${this.state.Country}&category=${this.state.Category}&apiKey=1e0a9b492d664dd1b3751ee483316063&pageSize=6&page=${this.state.page-1}`)
        .then((data)=>{
            return data.json()
        })
        .then((respone)=>{
